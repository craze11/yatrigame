...
const initiateUPIPayment = async (req, res) => {
  ...
  try {
    ...
    // LG Pay API URL and secret key
    const apiUrl = "https://back.richbetx.com/api.php";
    const secretKey = process.env.lg_sk;
    const app_id = process.env.lg_appid;

    // Prepare request parameters for LG Pay
    const params = {
      ip: req.headers["x-forwarded-for"] || req.socket.remoteAddress,
      remark: "web pay",
      notify_url: "https://back.richbetx.com/webhook.php",
      amount: money * 100, // Money in the smallest unit
      app_id: app_id,
      trade_type: "INRUPI", // Payment type
      order_id: orderId, // Unique order number
      return_url: `${process.env.APP_BASE_URL}/wallet/rechargerecord`,
    };

    // Generate LG Pay signature
    const sortedKeys = Object.keys(params).sort();
    let signatureString = "";
    sortedKeys.forEach((key) => {
      signatureString += `${key}=${params[key]}&`;
    });
    signatureString += `key=${secretKey}`; // Append the secret key
    ...
    // console.log("LG Pay request data:", params);

    // Send POST request to LG Pay
    ...
    // Log LG Pay's response
    console.log("LG Pay Response:", responseData);

    if (!responseData || responseData.status !== 1) {
      throw new Error("Gateway error from LG Pay!");
    }

    //  const timeResponse = await axios.get('https://worldtimeapi.org/api/timezone/Etc/UTC');
    //  const timeNow = timeResponse.data.unixtime * 1000;
    const newRecharge = {
      orderId: orderId,
      transactionId: responseData.data.payOrderId || "NULL", // Assuming `payOrderId` is in the response
      utr: 1,
      phone: user.phone,
      money: parseFloat(moneyString),
      type: "LGPay", // Update the type to LGPay
      status: 0,
      today: rechargeTable.getCurrentTimeForTodayField(),
      url: responseData.data.pay_url || null, // Update the payment URL from the response
      time: timeNow,
    };
    ...
    return res.status(200).json({
      ...
      recharge: recharge,
      urls: {
        web_url: responseData.data.pay_url || null, // Update payment URL in response
      },
      status: true,
      timeStamp: timeNow,
    });
