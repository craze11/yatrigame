  ...
  try {
    ...
    const [bets] = await connection.query(
      ...
      FROM (
        ...
        FROM minutes_1
        WHERE proxy = 0 AND time > ? AND time <= ?
        GROUP BY phone
        UNION ALL
        SELECT phone, SUM(money + fee) AS total_money
        FROM trx_wingo_bets
...
const betWinGo = async (req, res) => {
  ...
  const [user] = await connection.query(
    "SELECT `phone`, `code`, `invite`, `level`, `money`, `bonus_money`, `proxy` FROM users WHERE token = ? AND veri = 1  LIMIT 1 ",
    [auth],
  );
  ...
  try {
    ...
    if (isPendingGame) {
      ...
      const [minPlayers] = await connection.query(
        "SELECT * FROM minutes_1 WHERE proxy = 0 AND status = 0 AND game = ?",
        [join],
      );
      ...
      if (minPlayers.length >= 2) {
        ...
        const totalMoneyPromises = betColumns.map(async (column) => {
          ...
          const query = `
                   ...
                   FROM minutes_1
                   WHERE game = ? AND proxy = 0 AND status = 0 AND bet IN (${placeholders})
               `;
          ...
          betColumns.map(async (column) => {
            const [result] = await connection.query(
              `
                     SELECT SUM(money) AS total_money
                     FROM minutes_1
                     WHERE game = ? AND proxy = 0 AND status = 0 AND bet IN (?)
                     `,
              [join, column.bets],
            );
            return {
